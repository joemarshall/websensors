...
class MarkdownVisitor(ast.NodeVisitor):
    ...
    def parse_value(self, node):
        if hasattr(node, "value"):
            ...
            return "(" + ",".join([self.parse_value(x) for x in node.elts]) + ")"
        elif isinstance(node, ast.List):
            return "(" + ",".join([self.parse_value(x) for x in node.elts]) + ")"
        else:
            raise RuntimeError(f"Couldn't parse node:{node}")
    ...
    def get_sig(self, node):
        ...
        for n, d in enumerate(args.defaults):
            argList[defaultStart + n] += "=" + self.parse_value(d)
        if len(args.kwonlyargs) > 0:
            ...
            for k, d in zip(args.kwonlyargs, args.kw_defaults):
                if d == None:
                    ...
                    argList.append(self.get_arg(k) + "=" + self.parse_value(d))
        if self.short:
    ...
    def visit_Module(self, node):
        ...
        if not self.short:
            self.short = True
            print(
                f'<div id="{"_".join(self.parent_objects)}" class="moduletarget" markdown=1>'
            )
    ...
    def visit_ClassDef(self, node):
        ...
        if not self.short:
            ...
            print(
                f'<div id="{"_".join(self.parent_objects)}" class="classtarget" markdown=1>'
            )
            print(f"\n# class {class_name} \n")
    ...
    def visit_FunctionDef(self, node):
        ...
        if docstr and len(docstr) > 0:
            if self.short:
                ...
                print(
                    f'<a id="{"_".join(self.parent_objects+[node.name])}" class="fntarget"></a>'
                )
                ...
                for c in range(len(self.parent_objects)):
                    parentLinks.append(
                        f"[*{self.parent_objects[c]}*](#{ '_'.join(self.parent_objects[0:c+1])})"
                    )
    ...
    def visit_AsyncFunctionDef(self, node):
        ...
        if docstr and len(docstr) > 0:
            # only document things with valid docstrs
            print(
                ".".join(self.parent_objects + [self.get_sig(node)]),
                self.parse_docstring(node),
            )
        # don't visit inside functions - we assume sub-functions are private
        return
...
all_files = glob.glob("_includes/grove-base/*.py")
exclude_list = ["grove.*\.py", "pn532\.py", "outputcsv\.py", "showIP\.py"]
files = []
for x in all_files:
    basename = os.path.basename(x)
    included = True
    for patt in exclude_list:
        if re.search(patt, basename) is not None:
            included = False
            print(f"Skipping {x}")
            break
    if included:
...
for file in files:
    moduleName = re.match(".*/(.+)\.py", file).group(1)
